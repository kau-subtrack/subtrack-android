...
class DeliveryRepository @Inject constructor(
    ...
    fun jsonDelivery(json: DeliveryBaseResponse): List<DeliveryInfo>{
        ...
        for(pack in list) {
            val unit = DeliveryInfo(
                id = Random().nextInt(1000000), // 임시 고유 ID 생성 (정수)
    ...
    fun convertStatus(dataStatus: String): DeliveryStatus{
        return try {
            // 문자열을 ParcelStatus enum으로 변환
            val parcelStatus = ParcelStatus.valueOf(dataStatus)

            // ParcelStatus에 따라 적절한 DeliveryStatus로 매핑
            when (parcelStatus) {
                ParcelStatus.PENDING_PICKUP, ParcelStatus.IN_PICKUP -> DeliveryStatus.PENDING
                ParcelStatus.PICKUP_COMPLETED, ParcelStatus.PENDING_DELIVERY, ParcelStatus.IN_DELIVERY -> DeliveryStatus.IN_PROGRESS
                ParcelStatus.DELIVERY_COMPLETED -> DeliveryStatus.DELIVERED
            }
